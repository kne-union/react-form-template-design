...
import { FormItem, ImageComponent, TextComponent } from '../utils/formItem';
...
export default memo(({ configTarget, ...props }) => {
	const type = useMemo(() => (configTarget instanceof FormItem ? 'FormItem' : configTarget instanceof ImageComponent ? 'ImageComponent' : configTarget instanceof TextComponent ? 'TextComponent' : ''), [configTarget]);
...
const TextConfig = memo(
	({ configTarget, onChange, onSubmit }) => {
...
const FormItemConfig = memo(
	({ configTarget, onSubmit }) => {
		const classes = useStyles();
		...
		useEffect(() => {
			...
			reset({
				...
				placeholder: configTarget.placeholder,
				...
				{configTarget && (
					<div>
						<form onSubmit={handleSubmit(onSubmit)}>
							<div className='config_filed'>
								<TextField required fullWidth={true} variant="outlined" label="label" name="label" inputRef={register({ required: true })} size="medium" />
							</div>
							<div className='config_filed'>
								<TextField fullWidth={true} variant="outlined" label="placeholder" name="placeholder" inputRef={register({})} size="medium" />
							</div>
							...
							{configTarget.name === 'Select' && (
							 ...
							 <div className='config_filed'>
								<FormControl name="required">
									...
									<RadioGroup row name="required" onChange={e=>setValue('required',e.target.value) } defaultValue={String(configTarget.required)}>
