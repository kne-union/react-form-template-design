...
import { getItem, getItemFromObj, FormItem, ImageComponent, TextComponent } from './utils/formItem';
...
export default ( { onOver = console.log } ) => {
    ...
    }, [FormOption, getComponent, handleDragOver] );
    ...
    }, [SimpleOption, getComponent, handleDragOver] );
...
const FormItemView = ( { itm, configTarget, modConfigTarget } ) => {
    const type = useMemo( () => ( itm instanceof FormItem ? 'FormItem' : itm instanceof ImageComponent ? 'ImageComponent' : itm instanceof TextComponent ? 'TextComponent' : '' ), [itm] );
