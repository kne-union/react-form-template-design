...
export class FormItem extends Item {
	constructor( name ) {
		...
		const dateField = ['DateTimePicker', 'DatePicker']
		if ( dateField.some( itm => itm === name ) ) {
			this.isDateFormItem = true
		}
	}
...
export class StaticComponent extends Item {
	constructor( name ) {
		super( name )
		this.style = {
			height: '100%',
	...
	get formItemInfo () {
		return this._formItemInfo
	}

	static newFromObject ( obj ) {
		const target = new StaticComponent( obj.name );
		target.id = obj.id;
		target.style = obj.style;
		return target;
	}
...
export const getItem = name => {
	switch ( name ) {
		...
		case 'StaticCom':
			return new StaticComponent( name );
		default:
...
export const getItemFromObj = obj => {
	switch ( obj.name ) {
		...
		case 'Img':
			return ImageComponent.newFromObject( obj );
		case 'StaticCom':
			return StaticComponent.newFromObject( obj );
		default:
			return FormItem.newFromObject( obj );
	}
};
